...
Then, we begin writing.  Import the technical library like so on the first line of your .tex document: \\\\\codefull{\sla input\{../library\_technical\}}\\ The location of the file may be different, but make sure you have \code{library\_technical.tex} as input file.
...
Then, we import the notebook library like so on the first line of your .tex document:\\\\ \codefull{\sla input\{../library\_notebook\}}\\ The location of the file may be different, but make sure you have \code{library\_notebook.tex} as input file.

\section{Setting Up the Document}
Setting up a \LaTeX{} document is easy.  Add the following line of code so your document looks like this: \\\\\codefull{\sla input\{../library\_******\}\\\sla begin\{document\}\\\\\sla end\{document\}}\\\\  Notice that all \LaTeX{} commands start with a \code{\sla}.\pagebreak
\\In the \code{\sla begin\{document\} ... \sla end\{document\}} section, add the following.
\\\\\codefull{\color{red}{\%PART AND CHAPTER DETAILS - REPLACE HIGHLIGHTED!!!}\color{black}\\\sla renewcommand\{\sla currentpart\}\{LASA Robotics {\color{red}Technical} Documentation\}\\\sla renewcommand\{\sla currentchapter\}\{{\color{red}Topic, ex.Writing Documentation in LaTeX}\}\\\sla createtitle\{{\color{red}relative path to library folder}\}}\\\\
...
\subsection{Technical Documentation}
\codefull{\sla def \sla librarypath \{{\color{red}../}\}\space\space\%relative path to library\\\sla input\{\sla librarypath library\_technical\}\\\\
\sla begin\{document\}\\
...
\sla createtitle\{\sla librarypath\}\space\space\%relative path to library\\\\
\%-----INSERT CONTENT HERE-----\%\\\\
\sla end\{document\}}
\subsection{Notebook Documentation}
\codefull{\sla def \sla librarypath \{{\color{red}../}\}\space\space\%relative path to library\\\sla input\{\sla librarypath library\_notebook\}\\\\
\sla begin\{document\}\\
...
Environments run \LaTeX{}'s powerful processing.  With these, you can create anything from centered text to lists.  Remember that wherever there is a \code{\sla begin}, there must be an \code{\sla end}.
